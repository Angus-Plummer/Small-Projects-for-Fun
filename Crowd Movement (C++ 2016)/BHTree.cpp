...
Vector3D BHTreeNode::getVelocitySum(){
	return velocitySum;
}
...
void BHTreeNode::addParticle(Vector3D particlePos, Vector3D particleVel, double particleMass){
	...
	if (!isEmpty){
		...
		if (isExternal){
			...
			for (int i = 0; i < 8; i++){
				if (children[i]->contains(particlePos)){
					children[i]->addParticle(particlePos, particleVel, particleMass);
				}
				if (children[i]->contains(centreOfMass)){
					children[i]->addParticle(centreOfMass, velocitySum, mass);
				}
			}
		}
		//if it is an internal node
			...
			for (int i = 0; i < 8; i++){
				if (children[i]->contains(particlePos)){
					//std::cout << "child " << i << "contains particle\n";
					children[i]->addParticle(particlePos, particleVel, particleMass);
				}
			}
		}
